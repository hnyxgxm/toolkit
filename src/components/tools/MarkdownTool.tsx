...
import { PageHeader, CopyButton, downloadFile } from "@/components/ui";
...
function safeFileName(title: string): string {
  ...
  return base ? `${base}.md` : "markdown.md";
}
...
export default function MarkdownTool() {
  ...
  const onExport = useCallback(() => {
    downloadFile(fileName, input, "text/markdown;charset=utf-8");
  }, [fileName, input]);
        ...
        <div className="flex-1 min-w-0 w-full">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-1">
              <CopyButton text={input} label="复制 Markdown" />
              <button
                onClick={onExport}
                title={`导出 Markdown 文件：${fileName}`}
